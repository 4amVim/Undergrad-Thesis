...
This undergraduate thesis aims to explain the interpretation of Yoneda Lemma as
described in $CITE$.

We prove Yoneda Lemma, then derive Cayley's Theorem and Yoneda Embedding as consequences of its proof.
Then we define the categories QUnif and ProMod, and show functors between them.

%THE BELOW IS wrong and short, fix it
	...
	& X(K)
		...
		\begin{proof} \setcounter{equation}{0} Let a group $G$ be given.
			...
			& \mathcal{A} (\star,\star)
					...
					\end{equation}
					Where the last implication is given by the definition of composition in $\mathcal{A} $.
					Where, $\alpha_\star(1_\star)$ is a morphism in $A$ because $\alpha_\star$ is a function to $\mathcal{A} (\star,\star)$.
					Thus, every natural transformation $\alpha$ is defined in terms of
					its value at $1_\star$.
					This can be considered as left multiplication by
					$\alpha_\star(1_\star)$ in $G$, which
					we know to be an automorphism of $G$. Thus, $\alpha_\star$, and hence $\alpha$
					can be thought of as a bijection on $G$.

					So far we have shown we have that shown that the collection of
					all $\alpha:H_\star \to H_\star$,
					which is $[\mathcal{A} ^{op},Set](H_\star,H_\star)$ is a
				...
				\item\textbf{The collection $[\mathcal{A}^{op},Set](H_\star,H_\star)$ is a group.}
					...
					morphisms with the same source and destination, which is $H_\star$,
					we get that it is closed under composition, and has an identity.

					We will now show closure under inverses.
					Fix any $\gamma:H_\star \to H_\star$.
					Using I gives us the existence of $j\in G$ such that
					$j=\psi \big( \gamma_\star(1_\star) \big)$, which implies
					there exists $j^{-1}\in G$.

					From I, we know that each natural transformation
					is defined in terms of it's value
					at $1_\star$. That value is a morphism in $\mathcal{A}(\star,\star)$,
					and hence, gives an element of $G$.
					Thus, every element of $G$ gives us
					at least one natural transformation.
					Hence, as $j^{-1} \in G$, there exists $\delta:H_\star \to H_\star$ such that
					$j^{-1}=\psi \big(\delta_\star(1_\star) $.	And as $\psi$ is a group
					isomorphism,
					\[ 1_\star = \psi^{-1}(j.j^{-1})=\psi^{-1}(j)\,. \psi^{-1}(j^{-1})
...
This gives us that $\delta$ and $\gamma$ are inverses, as
				...
				\item \textbf{Applying Yoneda Lemma.}
					...
					Taking $A=\star$ and $X=H_\star$ in Theorem 2.6 (1), we get:
			...
			And $RHS$ being $\mathcal{A} (\star, \star)$, which is isomorphic to group $G$.
			This is	precisely the statement of Cayley's theorem:
		\[ Sym(G) \geq [ \mathcal{A} ^{op}, Set](H_\star,H_\star) \hat{\cong}
		\mathcal{A} (\star,\star) \overset{\psi}{\cong} G .\qedhere \]
		\end{proof}
		...
		& Y
			...
			For the definition of 2-category, we have referred to $CITE$.
