...
\begin{proof}\setcounter{equation}{0}
	...
	\end{lemma}
	\begin{proof} \setcounter{equation}{0}
	In order to show naturality of $\alpha_{(A,B)}$,
	\[ \text{ we need to show that
		for any }(f,g) \in \mathcal{A} \times \mathcal{B} \; (X,Y), \text{ the square }
	\begin{tikzcd}
	F(X) \arrow[swap]{d}{\alpha_X} \arrow{r}{F((f,g))}
		& F(Y) \arrow{d}{\alpha_Y}\\
	G(X) \arrow[swap]{r}{G((f,g))}
		& G(Y)
	...
	where $g \in \mathcal{A} (A,A')$ and $f \in \mathcal{B} (B,B')$.
		\begin{enumerate}[label=(\Roman*)]
			\item The condition (ii) gives us that
				...
				\end{tikzcd}
				$.
			\item The condition (i) gives us that
				...
				\end{tikzcd}
				$.
			\item Composing the squares from (I) and (II), we get that the following
				rectangle commutes:
