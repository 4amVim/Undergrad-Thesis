...
\begin{proof} \setcounter{equation}{0} Let a group $(G,.)$ be given.
	...
	be $f.g$.
	Thus, by definition of $\mathcal{A}$, $G$ and $Hom_{\mathcal{A}}^{}(\star, \star)$ are
	isomorphic as groups, becuase they have the same elements and rule of composition.
	And as $H_\star(\star)=\mathcal{A} (\star,\star)$ by definition, we get that as groups,
	$G \cong H_\star(\star)$. Hence, $G$ being a set gives us that $\mathcal{A}$ is a locally
	small category. Because $\mathcal{A} ^{op}$ has the same number of morphism as $\mathcal{A}$,
	...
	\begin{equation} [ \mathcal{A} ^{op}, Set](H_\star,H_\star) \cong H_\star(\star).\end{equation}
	where the above isomorphism is between sets. As $\mathcal{A} ^{op}$ has only one object,
	each natural transformation $\alpha:H_\star^{} \to H_\star^{}$ has only one component,
	$\alpha_\star$, which is a funtion $\mathcal{A} (\star,\star) \to \mathcal{A} (\star,\star)$.
	...
	\end{equation}
	Applying (2) to the identity $1_\star$ of $\star \in \mathcal{A} $
	gives us the following.
	\begin{equation}
		\big( (\string _ . f) \circ \alpha_\star \big)(1_\star) =
		\big(  \alpha_\star \circ (\string _ \circ f)\big)(1_\star)
		\implies \alpha_*(1_\star).f=\alpha_\star(f)
	\end{equation}
	Therefore, $\mathcal{A} (\star,\star)$ is actually G viewed as a right G-set. Thus, the only
	component of every natural transformation $\alpha:H_\star \to H_\star$,
	is exactly an automorphism on group $G$ obtained by right multiplication with an element of
	$G$. Hence, if we equate each $\alpha$ with $\alpha_\star$, then the left hand side
	of (1) becomes the group of automorphisms on $G$, which is a subgroup
	of the symmetric group on G. Now we will show that the isomorphism in (1) is also a
	homomorphism. The map $\string ^$, defined in Theorem 1.1 will act here as $\alpha
	\mapsto \alpha_\star (1_\star)$. Hence, for any natural transformations $\alpha,\beta:H_\star
	\to H_\star$,
	\begin{equation}
	...
	\end{equation}
	Where the last equality is due to $(\beta)_\star (1_\star)$ being an element of $\mathcal{A}
	(\star,\star)$, and hence corresponding to some element of G. \qedhere
\end{proof}
...
$\mathcal{A} $ as taking any oject $A \in \mathcal{A} $ to $H_A$ and
\begin{proop} %{{{H.		Definition
\end{proop}
...
\begin{proof}
	...
	full, faithful and injective on any category.
\end{proof}
