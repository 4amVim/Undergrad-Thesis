...
\end{definition}
%{{{Prorelations closed under composition
\begin{lemma}
...
\begin{proof}
	...
	\item (Up-Set) Let $l:X \to Z$ be a relation, and $k \in Q.P$ such that $l \supseteq k$.
		...
		$b \supseteq b'$ and $a \supseteq a'$. As $Q$ and $P$ are up-sets,
		we get that $b \in B$ and $a \in A$, and hence $b\circ a \in Q.P$.
		Finally, for $(x,z) \in X \times Z$, if $(x,z) \in l$, then $\forall y \in Y$,
		$(x,y) \in a \text{ and } (y,z) \in b $ gives that $(x,z) \in b \circ a$,
		giving that $l=b \circ a \in Q.P$.
	\end{enumerate}
\end{proof}
%}}}
...
\begin{proof}
	...
	$x \in X$, there exists some $(x,y) \in r$, and by definition of $f^o$ this also gives
	that $(y,x) \in f^o$. Hence, by	definition of composition,
	for every $x \in X$, $(x,x) \in f^o \,f$.
\end{proof}
\begin{lemma}
...
\begin{proof}
	...
	$x\,r\,y$ gives $x\,s\,y$. And via composition with $y \, t \, z $ we get
...
\begin{proof}
	...
	implies that $y\,t\,z$. Using composition again yields $x(t\,r)z$.
\end{proof}
%}}}
...
\end{proof}
%}}}
%{{{Two functors FIRST FUNCTOR UP-SET PROBLEM
...
\begin{proof}
	...
	In order to show $C.g.f \leq C.g.B.f$, fix any $b\in B, c\in C$.
			...
			Now, to show that $C.g.f \geq C.g.B.f$.
