...
This gives us that $\delta$ and $\gamma$ are inverses, as
		...
		Base $\mathbb{B}$ is a subset of $A$ contains a relation in $\mathbb{B}$.
	DEFINE BASE HERE
			\begin{prop}
				Let $\mathbb{B}$ be the base for quasi-uniformity $A$ on $X$. 				For $x \in X$, define $Ball(x)=\{B(x) | B \in \mathbb{B}\}$.
				Then there is a unique topology on $X$ such that for each $x\in X$,
				$Ball(x)$ is a base for the neighborhood of x in this topology.
			\end{prop}
