...
described in \cite{Clementino_Hofmann_2011}.
...
In this section, we set forth some basic definitions, and prove some lemmas that we will require in
		...
		\begin{proof} \setcounter{equation}{0} Let a group $G$ be given.
			...
			& \mathcal{A} (\star,\star)
				...
				\item\textbf{The collection $[\mathcal{A}^{op},Set](H_\star,H_\star)$ is a group.}
					...
					it is closed under composition. Also, identity of
					...
					Since $\gamma_\star(1_\star)$ belongs to $A(\star,\star)$,
					...
					\mathcal{A}(\star,\star)$. Hence, every morphism in
					$ \mathcal{A} (H_\star,H_\star)$ gives us a natural
					transformation. Thus, we can define
...
This gives us that $\delta$ and $\gamma$ are inverses, as
		...
		& Y
			...
			\begin{prop}
				Let $\mathbb{B}$ be the base for quasi-uniformity $A$ on $X$.That is,
				every relation in $A$ contains a relation in $\mathbb{B}$.
				For $x \in X$, define $B(x)=\{B(x) | B \in \mathbb{B}\}$.
				Then there is a unique topology on $X$ such that for each $x\in X$,
				$B(x)$ is a base for the neighborhood of x in this topology.
			\end{prop}
