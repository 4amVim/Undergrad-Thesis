...
\begin{proof}Let a locally small category,  $\mathcal{A} $ be given. \\
	...
	\paragraph{Defining $\phi$ and $\psi$}
	Define $\phi$ (on natural transformations) as : for $\alpha \in \mathcal{C}(H_A,X) ,
	\phi(\alpha):= \alpha_A(1_A) $ \\

	Define $\phi$ on an object, $ x \in X(A), $ by defining it's $K$-component for any $K \in \mathcal{A} $:
	$({\psi(x)})_K$
