...
\begin{example}{\textbf{Determinant (of an n$\times$n matrix)}} %{{{ Determinant (of an n$\times$n matrix)		Example
	...
	matrices with entries from it. Also, there's a forgetful functor, $F:\textit{CRing} \rightarrow \textit{Mon}$
...
M_n(R) \arrow[swap]{d}{det_R} \arrow{r}{M_n(h)}
	& M_n(S) \arrow{d}{det_S}\\
G(R) \arrow[swap]{r}{F(h)}
	& F(S)
\end{tikzcd}
\text{ must commute }
	\]
	So, need to show that, given a matrix $M$ over $R$, and $H:= M_n(h)\; ; \;$
	\[ |M| \]
